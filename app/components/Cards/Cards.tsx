        ...
        <div className="col-span-4 grid md:grid-cols-4 grid-cols-2 gap-4 order-2 md:order-0">
            {!isSearchBeingDone ? (
                cards.map((card, index) => (
                    <div key={index} className="bg-[#25253F] border-2 border-[#3C3B5D] rounded-lg flex flex-col items-start gap-6 p-4 min-w-0">
                        <h1 className="text-[#CECFE4] text-lg w-full truncate">{card.title}</h1>
                        <span className="text-3xl w-full truncate">{Number(card.value).toFixed()}{card.unit === "%" ? card.unit : ` ${card.unit}`}</span>
                    </div>
                ))
