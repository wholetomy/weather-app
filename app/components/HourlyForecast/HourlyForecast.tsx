...
export default function HourlyForecast({
    ...
    const [isHourlyForecastDropDownOpened, setIsHourlyForecastDropDownOpened] = useState(false);
    const [whichDayIsClicked, setWhichDayIsClicked] = useState("");
    ...
    useEffect(() => {
        ...
        return () => {
            document.removeEventListener("mousedown",FecharModalDeDiasSemanaisAoClicarFora);
        };
    }, []);
        ...
        return (
            <div className="md:col-span-2 md:row-span-3 bg-[#25253F] order-4 md:order-0 p-4 h-125 md:h-full overflow-y-auto scrollbar-thin scrollbar-thumb-[#3B3A5C] scrollbar-track-[#25253F]">
                <div className="flex justify-between items-center flex-wrap">
                    ...
                    <button className="bg-[#3C3B5B] rounded-sm flex gap-2 items-center py-1 px-2">
                        <span>-</span>
                        <IoIosArrowDown />
                    </button>
                </div>
                ...
                <div className="flex gap-2 flex-col mt-2">
                    {Array.from({ length: 24 }).map((_, index) => (
                        <div
                            key={index}
                            className="bg-[#2F2F49] w-full h-10 rounded-sm animate-pulse [animation-duration:2s]"
                        />
                    ))}
                </div>
            </div>
            ...
            <div className="flex justify-between items-center flex-wrap">
                ...
                >
                    <button
                        ...
                        onClick={() =>
                            setIsHourlyForecastDropDownOpened(
            ...
            <div className="flex gap-2 flex-col mt-2">
                {informacoesMeteorologicas.hourly.time.map((time, index) => {
                    ...
                    return (
                        <div
                            key={index}
                            className="bg-[#2F2F49] w-full h-10 rounded-sm flex items-center justify-between gap-4"
                        >
